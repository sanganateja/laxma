SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[PAYMENT_MES_IN_QUEUE_CREATE]
    @p_amount_minor_units NUMERIC,
    @p_currency_code_alpha3 CHAR(3),
    @p_end_to_end_id NVARCHAR(255),
    @p_header_message_id NVARCHAR(35),
    @p_instruction_for_next_agent NVARCHAR(140),
    @p_intermediary_agent_id NVARCHAR(2000),
    @p_intermediary_agent_id_type VARCHAR(2000),
    @p_message_text NVARCHAR(MAX),
    @p_message_timestamp DATETIME2(6),
    @p_message_transaction_id NVARCHAR(20),
    @p_payee_account_id NVARCHAR(35),
    @p_payee_account_id_type CHAR(1),
    @p_payee_address_1 NVARCHAR(255),
    @p_payee_address_2 NVARCHAR(255),
    @p_payee_address_3 NVARCHAR(255),
    @p_payee_name NVARCHAR(255),
    @p_payee_sort_code NVARCHAR(35),
    @p_payer_account_id NVARCHAR(35),
    @p_payer_account_id_type CHAR(1),
    @p_payer_address_1 NVARCHAR(255),
    @p_payer_address_2 NVARCHAR(255),
    @p_payer_address_3 NVARCHAR(255),
    @p_payer_agent_id NVARCHAR(35),
    @p_payer_agent_id_type CHAR(1),
    @p_payer_name NVARCHAR(255),
    @p_payer_sort_code NVARCHAR(35),
    @p_payment_or_return CHAR(1),
    @p_payment_type CHAR(2),
    @p_reference_for_beneficiary NVARCHAR(35),
    @p_rmt_inf_addtl NVARCHAR(140),
    @p_rmt_inf_ustrd NVARCHAR(140),
    @p_settlement_date DATETIME2(6),
    @p_message_id NUMERIC
AS
BEGIN
    INSERT dbo.ACC_PAYMENT_MESSAGES_IN_QUEUE
    (
        MESSAGE_ID,
        MESSAGE_TIMESTAMP,
        PAYMENT_TYPE,
        HEADER_MESSAGE_ID,
        END_TO_END_ID,
        PAYER_NAME,
        PAYER_ACCOUNT_ID,
        PAYER_ADDRESS_1,
        PAYER_ADDRESS_2,
        PAYER_ADDRESS_3,
        PAYEE_NAME,
        PAYEE_ACCOUNT_ID,
        PAYEE_ADDRESS_1,
        PAYEE_ADDRESS_2,
        PAYEE_ADDRESS_3,
        AMOUNT_MINOR_UNITS,
        CURRENCY_CODE_ALPHA3,
        MESSAGE_TEXT,
        PAYMENT_OR_RETURN,
        PAYER_SORT_CODE,
        PAYEE_SORT_CODE,
        INSTRUCTION_FOR_NEXT_AGENT,
        PAYEE_ACCOUNT_ID_TYPE,
        PAYER_ACCOUNT_ID_TYPE,
        PAYER_AGENT_ID,
        PAYER_AGENT_ID_TYPE,
        SETTLEMENT_DATE,
        MESSAGE_TRANSACTION_ID,
        REFERENCE_FOR_BENEFICIARY,
        RMT_INF_ADDTL,
        RMT_INF_USTRD,
        INTERMEDIARY_AGENT_ID,
        INTERMEDIARY_AGENT_ID_TYPE
    )
    VALUES
    (@p_message_id,
     @p_message_timestamp,
     @p_payment_type,
     @p_header_message_id,
     @p_end_to_end_id,
     @p_payer_name,
     @p_payer_account_id,
     @p_payer_address_1,
     @p_payer_address_2,
     @p_payer_address_3,
     @p_payee_name,
     @p_payee_account_id,
     @p_payee_address_1,
     @p_payee_address_2,
     @p_payee_address_3,
     @p_amount_minor_units,
     @p_currency_code_alpha3,
     @p_message_text,
     @p_payment_or_return,
     @p_payer_sort_code,
     @p_payee_sort_code,
     @p_instruction_for_next_agent,
     @p_payee_account_id_type,
     @p_payer_account_id_type,
     @p_payer_agent_id,
     @p_payer_agent_id_type,
     @p_settlement_date,
     @p_message_transaction_id,
     @p_reference_for_beneficiary,
     @p_rmt_inf_addtl,
     @p_rmt_inf_ustrd,
     @p_intermediary_agent_id,
     @p_intermediary_agent_id_type);
END;
GO
