...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[ACC_REPORTS_PACKAGE$rpt_mthly_trad_limit_exceeded]	
                   ...
                   FROM ACC_OWNERS m	
                       ...
                       JOIN ACC_TRANSFERS tr	
                           ON tr.ACCOUNT_ID = ac.ACCOUNT_ID	
                              ...
                              AND tr.TRANSFER_TIME >= DATEADD(M, DATEDIFF(M, 0, GETDATE())-1 ,0)  -- First day of previous month	
                              AND tr.TRANSFER_TIME <  DATEADD(M, DATEDIFF(M, 0, GETDATE()) ,0)    -- First day of this month	
                   WHERE m.OWNER_ID = b.OWNER_ID	
                         AND ag.ACCOUNT_GROUP_TYPE = 'A' -- merchant account group	
               ) AS [Trade In GBP]	
        FROM ACC_OWNERS b	
    ) AS c	
    WHERE [Trade In GBP] > [Trading Limit]	
    ORDER BY [Excess],	
             [Excess %age],	
             [Business];	
END;
