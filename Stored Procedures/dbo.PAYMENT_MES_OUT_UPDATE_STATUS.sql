...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[PAYMENT_MES_OUT_UPDATE_STATUS]
    @cv_1 VARCHAR(2000) OUTPUT,
    @p_message_id NUMERIC,
    @p_message_status VARCHAR(2000)
AS
BEGIN

    SET @cv_1 = NULL;

    UPDATE dbo.ACC_PAYMENT_MESSAGES_OUT
    SET MESSAGE_STATUS = @p_message_status
    FROM dbo.ACC_PAYMENT_MESSAGES_OUT AS pm
    WHERE pm.MESSAGE_ID = @p_message_id;


    /*
      *    Return current state of message.
      *    This is not used, but is required for old versions of hibernate that cannot
      *    handle an update (since it doesn't return anything normally).
      */
