...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[ACC_REPORTS_PACKAGE$rpt_monthly_costs_incurred] @p_recordset VARCHAR(2000) OUTPUT
AS
BEGIN
    ...
    EXECUTE dbo.ACC_REPORTS_PACKAGE$rpt_period_costs_incurred @p_recordset = @p_recordset OUTPUT,
                                                              @p_startdate = @p_startdate,
                                                              @p_enddate = @p_enddate,
                                                              @p_owner_id = 0; /* report for all businesses*/
